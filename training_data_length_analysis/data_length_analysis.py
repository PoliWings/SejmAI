...
print(f"Min: {train_min_len}, Max: {train_max_len}, Average: {train_avg_len}, Median: {train_median_len}, Standard Deviation: {train_std_len}, 90th percentile: {train_90th_percentile}")

# Plot histogram for the training dataset token lengths
plt.figure(figsize=(12, 6))

# Plot for train dataset
plt.hist(train_lengths, bins=100, color='deepskyblue', edgecolor='black', linewidth=0.5, label=f'Min: {train_min_len}, Max: {train_max_len}, Std: {train_std_len:.2f}')
plt.title(f'Histogram of Token Lengths (Training data {filename.split("_")[0]})')
plt.xlabel('Token Length')
plt.ylabel('Frequency')
plt.axvline(train_90th_percentile, color='red', linestyle='dashed', linewidth=1, label=f'90th Percentile: {train_90th_percentile:.2f}')
plt.axvline(train_avg_len, color='navy', linestyle='dashed', linewidth=1, label=f'Average: {train_avg_len:.2f}')
plt.axvline(train_median_len, color='magenta', linestyle='dashed', linewidth=1, label=f'Median: {train_median_len:.2f}')
plt.legend()
